...
function qdot_joint = joint_limit_constraint(z, p, rest_coeff2, fric_coeff2, jointConstraint1, jointConstraint2, bodyConstraint)
    ...
    th5 = z(7);   % Body angle (newly added)
    ...
    dth5 = z(14); % Body angle velocity (newly added)
    ...
    for idx = 1:length(jointAngles)
        ...
        if joint_idx ~= 7
            if (angle < jointConstraint1 && velocity < 0)
            ...
            if (angle > jointConstraint2 && velocity > 0)
                % Define Jacobian for the current joint
                J = zeros(1, length(qdot_joint));
                J(joint_idx) = 1;
...
function [th1_des, th2_des, th3_des, th4_des] = get_desired_joint_angles(t, genome)
    ...
    hip_amplitude_right = genome(3);
    knee_amplitude_left = genome(4);
    knee_amplitude_right = genome(5);
    phaseOffset = genome(6);

    % Interpolate desired angles
    th1_des = -pi/4 + hip_amplitude_left * cos(omega * t);             % Left hip follows a cosine wave around initial angle
    th2_des = pi/2 + knee_amplitude_left * sin(omega * t);             % Left knee follows a sine wave around initial angle
    th3_des = -pi/4 + hip_amplitude_right * cos(omega * t + phaseOffset); % Right hip, 180 degrees out of phase
    th4_des = pi/2 + knee_amplitude_right * sin(omega * t + phaseOffset); % Right knee, 180 degrees out of phase
...
function obj = objective_function(genome)
    try
        ...
        distance = abs(simulate_robot(genome, false, 0.001, 20));
        if isnan(distance) || isinf(distance)
            obj = 1e6;  % Penalty for invalid distance
        else
            obj = -abs(distance);  % Negative because ga minimizes
        end
    catch
        obj = 1e6;  % Penalty for simulation failure
    end
end
function distance = simulate_robot(genome, do_animate, dt, tf)
    ...
    % joint
    rest_coeff2 = 0.;
    jointConstraint1 = deg2rad(-50);
    fric_coeff2 = 5;
    jointConstraint2 = deg2rad(50);
    ...
    for i = 1:num_steps-1
        ...
        z_out(8:14,i+1) = joint_limit_constraint(z_out(:,i+1), p, rest_coeff2, fric_coeff2, jointConstraint1, jointConstraint2, bodyConstraint);
        z_out(8:14,i+1) = discrete_impact_contact(z_out(:,i+1), p, rest_coeff, fric_coeff, ground_height);

        % State Update
        z_out(1:7, i+1) = z_out(1:7, i) + z_out(8:14,i+1) * dt;
        ...
        % rD_right = keypoints(:, 9);       % Intermediate point (D) on right leg
        % 
        % if rO_left(2) < 0 || rA_left(2) < 0 || rB_left(2) < 0 || rC_left(2) < 0 || rD_left(2) < 0 || rOR(2) < 0 || rA_right(2) < 0 || rB_right(2) < 0 || rC_right(2) < 0 || rD_right(2) < 0
...
%x0 = [-15, pi/8, pi/8, pi/12, pi/12, pi, 4, 4, 0.2, 0.2];  % Start from the middle of the bounds
%simulate_robot(x0, true, 0.001, 20);
optimize_robot();
